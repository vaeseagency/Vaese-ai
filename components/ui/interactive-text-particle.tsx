...
interface Particle {
  ...
  rgb: number[]
  draw(): void
  move(interactionRadius: number, hasPointer: boolean): boolean | undefined
}
...
const ParticleTextEffect: React.FC<ParticleTextEffectProps> = ({
  ...
  const rand = (max = 1, min = 0): number => min + Math.random() * (max - min)
  ...
  class ParticleClass implements Particle {
    ...
    constructor(x: number, y: number, rgb: number[] = [rand(128), rand(128), rand(128)]) {
      ...
      this.or = rand(5, 1); this.cr = this.or; this.pv = 0; this.ov = 0
  ...
  const dottify = () => {
    ...
    const pixels = data.reduce((arr: any[], _, i, d) => {
      if (i % 4 === 0) arr.push({ x: (i / 4) % textBox.w!, y: Math.floor((i / 4) / textBox.w!), rgb: d.slice(i, i + 4) })
      return arr
  ...
  const write = () => {
    ...
    ctx.font = `900 ${textBox.h}px Arial, sans-serif`
  ...
  useEffect(() => { initialize() }, [text, colors, animationForce, particleDensity, canvasSize])
